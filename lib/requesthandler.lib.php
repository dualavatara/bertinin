...
abstract class RequestMatcher implements IRequestMatcher{
	...
	protected  $authController = array(
        ...
        self::CHECK_USER_SESSION                    => 'checkUserSession'
	);
    ...
    protected function checkUserSession(DIContainer $di) {

        if( Session::obj() ){
            Session::obj()->__destroy();

            if( $this->check_vk_auth() ){
                $key = $this->salted($_GET['hash']);
                Session::obj()->__set($key, $_GET['uid']);
                setcookie('uid', $key);
            }
        }
        ...
        if( isset($_GET['delsession']) ){
            ...
            Session::obj()->__unset( Cookie::obj()->__get('uid') );
            Cookie::obj()->__unset('uid');
        }

        //echo '<div style="display:block; float:left; margin:25px; padding:5px; border:1px solid gray">' . Session::obj()->__get( Cookie::obj()->__get('uid') ) . '<br />' . Cookie::obj()->__get('uid') . '</div>';

        return true;
    }

    private function salted($clean){
        return md5($clean . '~salted~');
    }

    private function check_vk_auth(){
        return $_GET['hash'] ==  md5( '3948471' . $_GET['uid'] . 'r3DOTb55DdNOIM3op1RY' );
    }
